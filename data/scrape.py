...
def get_course_reviews(course):
    ...
    liked_rating = (
        soup.find("div", attrs={"class": "sc-psQdR iYtUny"}).text
        if soup.find("div", attrs={"class": "sc-psQdR iYtUny"}).text[-1] == "%"
        else ""
    )
    comments_count = int(
        re.findall(r"\d+", soup.find("a", attrs={"class": "sc-qPwPv gjSZrg"}).text)[0]
    )
    ratings_count = int(
        re.findall(r"\d+", soup.find("div", attrs={"class": "sc-qXRQq hXXFXN"}).text)[0]
    )
    comments = []
    ...
    if comments_count > 0:
        ...
        for commentCard in soup.findAll("div", attrs={"class": "sc-pcZJD cnFfnA"}):
            item = {
                "comment": commentCard.find(
                    "div", attrs={"class": "sc-pLwIe kqSAIH"}
                ).text
                if commentCard.find("div", attrs={"class": "sc-pLwIe kqSAIH"})
                else "",
                "author": commentCard.find(
                    "div", attrs={"class": "sc-pTWqp kvWxGq"}
                ).text
                if commentCard.find("div", attrs={"class": "sc-pTWqp kvWxGq"})
                else "",
            }
...
def get_course_term_offered(course):
    ...
    driver.get(url)
    WebDriverWait(driver, 20).until(
        EC.visibility_of_element_located((By.CSS_SELECTOR, ".sc-pAyMl.dPzwlT"))
    )
